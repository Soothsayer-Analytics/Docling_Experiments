...
def process_single_document_thread_safe(file_info_and_config):
    """
    Thread-safe version of document processing
    Takes a tuple of (file_info, export_type, azure_config)
    """
    try:
        file_path, filename, export_type, azure_config = file_info_and_config
        start_time = time.time()
        
        # Set Azure config in this thread's environment
        for key, value in azure_config.items():
            os.environ[key] = value
...
def process_documents_parallel_improved(file_paths_info, export_type, logger):
    """
    Improved parallel processing using ThreadPoolExecutor instead of ProcessPoolExecutor
    This is more compatible with Streamlit and handles complex objects better
    """
    logger.info(f"Starting thread-based parallel processing of {len(file_paths_info)} documents using {NUM_WORKERS} workers")
    
    all_splits = []
    processing_stats = {}
    
    # Prepare data for threads (include Azure config)
    thread_data = [
        (file_path, filename, export_type, AZURE_CONFIG)
        for file_path, filename in file_paths_info
    ]
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
            # Submit all tasks
            future_to_file = {
                executor.submit(process_single_document_thread_safe, data): data[1]
...
def process_documents_sequential_fallback(file_paths_info, export_type, logger):
    """
    Fallback to sequential processing if parallel processing fails
    """
    logger.info(f"Fallback: Processing {len(file_paths_info)} documents sequentially")
    ...
    try:
        for i, (file_path, filename) in enumerate(file_paths_info):
            try:
                data = (file_path, filename, export_type, AZURE_CONFIG)
                result = process_single_document_thread_safe(data)
                ...
                if result['success']:
                    ...
                    logger.error(
                        f"❌ {result['filename']}: Failed - {result['error']}"
                    )
            ...
            progress_bar.progress((i + 1) / len(file_paths_info))
            status_text.text(f"Processed {i + 1}/{len(file_paths_info)} files...")
...
def run_with_event_loop(func, *args, **kwargs):
    """Run a function with proper event loop setup"""
    try:
        loop = asyncio.get_event_loop()
    except RuntimeError:
        loop = asyncio.new_event_loop()
...
def load_database(db_name):
    ...
    try:
        with st.spinner(f"Loading database '{db_name}'..."):
            ...
            # Use the event loop wrapper
            vectorstore = run_with_event_loop(
                load_vector_store, 
                db_name, 
...
def process_new_database(db_name, uploaded_files, export_type):
    """Process and create a new database with improved parallel processing"""
    try:
        # Setup logging
        ...
        with st.spinner("Processing documents and creating database..."):
            ...
            if file_paths_info:
                ...
                st.info(f"🔄 Processing {len(file_paths_info)} documents using {NUM_WORKERS} parallel threads...")
                
                try:
                    splits, processing_stats = process_documents_parallel_improved(file_paths_info, export_type, logger)
                except Exception as e:
                    logger.warning(f"Parallel processing failed: {str(e)}")
                    logger.info("Falling back to sequential processing...")
                    st.warning("Parallel processing failed, falling back to sequential processing...")
                    splits, processing_stats = process_documents_sequential_fallback(file_paths_info, export_type, logger)
                ...
                if splits:
                    ...
                    if vectorstore:
                        ...
                        with st.expander("📈 Processing Summary", expanded=True):
                            ...
                            with col4:
                                st.metric("Threads Used", NUM_WORKERS)
                            
                            # Show thread usage breakdown
                            st.markdown("**Thread Usage Breakdown:**")
                            thread_usage = {}
                            for filename, stats in processing_stats.items():
                                thread_id = stats.get('thread_id', 'unknown')
                                if thread_id not in thread_usage:
                                    thread_usage[thread_id] = []
                                thread_usage[thread_id].append({
                                    'filename': filename,
                            ...
                            for thread_id, files in thread_usage.items():
                                st.markdown(f"**Thread-{thread_id}:**")
                                for file_info in files:
                                    status = "✅" if file_info['success'] else "❌"
                                    st.write(f"  {status} {file_info['filename']}: {file_info['time']:.2f}s ({file_info['chunks']} chunks)")
