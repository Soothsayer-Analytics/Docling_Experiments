...
AZURE_CONFIG = {
    "AZURE_OPENAI_ENDPOINT": os.getenv("AZURE_OPENAI_ENDPOINT", "https://oai-nasco.openai.azure.com"),
...
PDF_FOLDER_PATH = str(Path("Data").absolute())  # Changed to local Data folder
DATABASE_NAME = "Final"     # Change this to your desired database name
LOAD_DATABASE_NAME = "Final"       # Set this to load existing database instead of creating new one
SINGLE_QUERY = "What is the main topic of these documents?"                # Set this to execute a single query
USE_INTERACTIVE_MODE = False        # Set to True to start interactive mode after processing
...
class DocumentProcessor:
    ...
    def create_and_save_vector_store_with_rate_limiting(self, splits, embedding, db_name, batch_size=50, delay_between_batches=60):
        """Create and save vector store with rate limiting for API quotas using FAISS"""
        try:
            ...
            db_path = DATABASE_DIR / f"{db_name}_faiss"
            
            # Initialize empty FAISS vector store with first batch to define structure
            vectorstore = None
    ...
    def load_vector_store(self, db_name, embedding):
        """Load existing vector store (FAISS)"""
        try:
            ...
            # Check for FAISS folder
            db_path = DATABASE_DIR / f"{db_name}_faiss"
            
            if not db_path.exists():
                self.logger.warning(f"Database folder not found: {db_path}")
                return None
            
            # Load FAISS index with dangerous deserialization enabled (safe for local files)
